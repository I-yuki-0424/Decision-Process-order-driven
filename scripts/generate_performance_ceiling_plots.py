"""
Script to render performance ceiling plots: 22 Achievement Heatmap, Crafter Score Scaling, Z/Beam Width Tradeoffs, & Computational Bottleneck Radar.
"""
...
with open("output/performance_ceiling_exploration_metrics.json", "r") as f:
    data = json.load(f)

# -------------------------------------------------------------
# PLOT 20: 22 Achievements Unlock Rate Heatmap Across Model Depths (4, 8, 12, 16 Layers)
# -------------------------------------------------------------
fig, ax = plt.subplots(figsize=(12, 8))

achievements = data["all_22_achievements"]
...
cbar.ax.set_ylabel("Achievement Unlock Rate (%)", rotation=-90, va="bottom", fontweight='bold', fontsize=11)
...
plt.title("20. All 22 Craftax Achievement Unlock Rates Heatmap Across Model Depths", fontsize=14, fontweight='bold')
plt.tight_layout()
p20_path = "output/plots/Run-Seq-020_22_achievements_unlock_heatmap.png"
plt.savefig(p20_path, dpi=300)
plt.close()
print(f"Saved: {p20_path}")

# -------------------------------------------------------------
# PLOT 21: Layer Count vs. Step Scale Horizon Crafter Score (S_crafter)
# -------------------------------------------------------------
...
ax.set_ylabel('Overall Crafter Score (S_crafter)', fontsize=11, fontweight='bold')
ax.set_title('21. Crafter Score (S_crafter) Progression Across Training Horizons (8-Layer Model)', fontsize=13, fontweight='bold')
ax.set_ylim(60, 100)
ax.grid(True, linestyle='--', alpha=0.5)
ax.legend(loc='lower right')

plt.tight_layout()
p21_path = "output/plots/Run-Seq-021_layer_vs_step_scale_crafter_score.png"
plt.savefig(p21_path, dpi=300)
plt.close()
print(f"Saved: {p21_path}")

# -------------------------------------------------------------
# PLOT 22: Z-Compression & Beam Width k SPS Throughput Trade-off
# -------------------------------------------------------------
...
ax1_sub.set_ylabel('Crafter Score (S_crafter)', color=color2, fontweight='bold', fontsize=11)
ax1_sub.tick_params(axis='y', labelcolor=color2)
ax1_sub.set_ylim(60, 85)
...
ax2_sub.set_ylabel('Crafter Score (S_crafter)', color=color3, fontweight='bold', fontsize=11)
...
plt.suptitle("22. Z-Compression Frequency & Beam Width k Throughput Trade-offs", fontsize=13, fontweight='bold')
plt.tight_layout()
p22_path = "output/plots/Run-Seq-022_z_compression_vs_beam_k_sps_tradeoff.png"
plt.savefig(p22_path, dpi=300)
plt.close()
print(f"Saved: {p22_path}")

# -------------------------------------------------------------
# PLOT 23: Computational Bottleneck Radar Chart
# -------------------------------------------------------------
categories = ['Memory Footprint', 'FLOPs / Step', 'Attention Latency', 'Beam Branching Overhead', 'SPS Throughput']
N = len(categories)

# Scaled bottleneck scores across configurations (Normalized % of maximum load)
...
plt.title('23. Computational Load & Bottleneck Radar Across Architectures', size=12, fontweight='bold', y=1.08)
plt.legend(loc='upper right', bbox_to_anchor=(1.25, 1.1))

plt.tight_layout()
p23_path = "output/plots/Run-Seq-023_computational_bottleneck_radar.png"
plt.savefig(p23_path, dpi=300)
plt.close()
print(f"Saved: {p23_path}")

print("All performance ceiling plots generated successfully in output/plots/")
