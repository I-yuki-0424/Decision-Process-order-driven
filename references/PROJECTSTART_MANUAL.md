...
| --------------- | -------------------------------------------- | --------------- | ------------------------------------------------- |
| Gemini CLI       | `--yolo`                                     | full disable    | Prefer this if present; fallback to `--dangerously-bypass-approvals-and-sandbox` or `--full-auto` |
...
* MUST call `scripts/print_banner.sh` first.
* MUST prefer `--yolo` if supported, with fallbacks.
* MUST fallback to `--full-auto` if the dangerous flag is not supported.

Feature-detect template:

```bash
# Highest privilege if the flag exists; otherwise fallback.
...
* [ ] `scripts/tk_start_gemini.sh` uses `--yolo` if supported, else fallback
