...
def build_decision_attention_mask(num_actions: int, num_hist: int) -> jnp.ndarray:
    ...
    # 3. History sequence tokens: attend to State/Target (:2), and causal history among themselves
    mask = mask.at[2 + num_actions:, :2].set(1.0)
    hist_causal = jnp.tril(jnp.ones((num_hist, num_hist), dtype=jnp.float32))
    mask = mask.at[2 + num_actions:, 2 + num_actions:].set(hist_causal)
...
def forward_decision_transformer(
    ...
    attn_mask = build_decision_attention_mask(num_actions, num_hist)
