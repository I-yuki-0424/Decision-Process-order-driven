...
def init_model_parameters(
    ...
    h_keys = jax.random.split(keys[-1], 5)
    heads = HeadParameters(
...
def forward_layer(
    ...
    """Forward pass through a single Multi-Head Attention layer with optional KV Cache."""
...
def forward_decision_transformer(
    ...
    # 4. Pooling / Aggregation (Use final token representation)
